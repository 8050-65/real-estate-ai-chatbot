...
(function() {
  ...
  container.innerHTML = `
    ...
    <div id="leadrat-chatbot-window" class="leadrat-chatbot-window" style="display: none;">
      ...
      <iframe id="leadrat-chatbot-iframe"
        src="${config.chatbotUrl}/chat-ui.html?embedded=true"
        frameborder="0"
  ...
  button.addEventListener('click', () => {
    const isOpening = window_.style.display === 'none';
    window_.style.display = isOpening ? 'flex' : 'none';
    button.style.opacity = isOpening ? '0.5' : '1';
    
    if (isOpening) {
      const iframe = document.getElementById('leadrat-chatbot-iframe');
      iframe.contentWindow.postMessage({
        ...
        config: {
          ...
          tenantId: 'dubait11',
          botName: 'Aria AI',
          botSubtitle: 'Executive Real Estate Assistant'
        }
      }, '*');
    }
  });
